...
class HttpProxyMiddleware(object):
    ...
    def __init__(self, settings):
        ...
        # 每隔固定时间强制抓取新代理(min)
        self.fetch_proxy_interval = 5
        # 一个将被设为invalid的代理如果已经成功爬取大于这个参数的页面， 将不会被invalid
        self.invalid_proxy_threshold = 10000000
        # 从文件读取初始代理
    ...
    def fetch_new_proxyes(self):
        ...
        new_proxyes = httpcall.fetch_all()
    ...
    def process_request(self, request, spider):
        ...
        if "through" in request.meta.keys() and request.meta["through"]:
            logger.info("Through request get by spider: %s"  % request)
            self.proxy_index = 0
